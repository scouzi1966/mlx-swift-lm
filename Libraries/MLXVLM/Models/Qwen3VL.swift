...
enum Qwen3VLLanguage {
    ...
    final class RotaryEmbedding {
        ...
        private func applyInterleavedMRope(_ freqs: MLXArray) -> MLXArray {
            // Optimized implementation using batch operations instead of 64 individual array creations
            // freqs shape: (3, bs, seq_len, head_dim // 2) where dim 0 is [temporal, height, width]
            let dims = freqs.dim(-1)  // typically 64
            let hEnd = min(mropeSection[1] * 3, dims)  // height section boundary
            let wEnd = min(mropeSection[2] * 3, dims)  // width section boundary

            // Calculate number of complete triplets we can process
            // Each triplet is [temporal, height, width] at positions [3k, 3k+1, 3k+2]
            let numTriplets = min(hEnd, wEnd) / 3

            if numTriplets == 0 {
                // Edge case: no interleaving needed, just return temporal
                return freqs[0, 0..., 0..., 0...]
            }

            // Pre-compute stride indices for batch gathering
            // Temporal: [0, 3, 6, 9, ...], Height: [1, 4, 7, 10, ...], Width: [2, 5, 8, 11, ...]
            let tIdx = MLXArray(stride(from: 0, to: numTriplets * 3, by: 3).map { Int32($0) })
            let hIdx = MLXArray(stride(from: 1, to: numTriplets * 3, by: 3).map { Int32($0) })
            let wIdx = MLXArray(stride(from: 2, to: numTriplets * 3, by: 3).map { Int32($0) })

            // Batch gather from each source dimension (3 operations instead of 64)
            let tSlices = take(freqs[0, 0..., 0..., 0...], tIdx, axis: -1)
            let hSlices = take(freqs[1, 0..., 0..., 0...], hIdx, axis: -1)
            let wSlices = take(freqs[2, 0..., 0..., 0...], wIdx, axis: -1)

            // Interleave by stacking [t, h, w] and reshaping
            // stacked shape: (bs, seq_len, numTriplets, 3)
            let interleaved = stacked([tSlices, hSlices, wSlices], axis: -1)
            let bs = freqs.dim(1)
            let seqLen = freqs.dim(2)
            var result = interleaved.reshaped(bs, seqLen, numTriplets * 3)

            // Handle remaining positions after complete triplets (use temporal frequencies)
            let remaining = dims - numTriplets * 3
            if remaining > 0 {
                let remainingSlice = freqs[0, 0..., 0..., (numTriplets * 3)...]
                result = concatenated([result, remainingSlice], axis: -1)
            }
