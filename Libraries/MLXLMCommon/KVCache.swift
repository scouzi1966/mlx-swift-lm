...
public class KVCacheSimple: BaseKVCache, CustomDebugStringConvertible {
    ...
    /// Initial allocation size (tokens). After this, capacity doubles on each resize.
    public var step = 256
    ...
    public override func update(keys: MLXArray, values: MLXArray) -> (MLXArray, MLXArray) {
        ...
        if needsGrow {
            ...
            // 2x growth strategy (like std::vector): amortized O(1) copies.
            // Start with at least `step` tokens, then double current capacity.
            let currentCapacity = self.keys?.dim(2) ?? 0
            let newCapacity: Int
            if currentCapacity == 0 {
                // First allocation: round up to step boundary
                let nSteps = (newTokens + step - 1) / step
                newCapacity = nSteps * step
            } else {
                // Double capacity until it fits
                var cap = currentCapacity * 2
                while cap < needed {
                    cap *= 2
                }
                newCapacity = cap
            ...
            let growBy = newCapacity - currentCapacity
            let kShape = [B, kvHeads, growBy, kHeadDim]
            let vShape = [B, kvHeads, growBy, vHeadDim]
            ...
            // Signal that array references changed — compile() state tracking needs refresh
            didGrow = true
        }
