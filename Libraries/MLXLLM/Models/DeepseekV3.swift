...
// MARK: - MultiLinear (per-head linear with quantization support)

/// Per-head linear layer with weight shape [numHeads, outputDims, inputDims].
/// Supports both regular and quantized weights. The weight, scales, and biases
/// are initialized as scalar placeholders; actual tensors come from checkpoint
/// via update(parameters:). Quantized models store packed weights with shapes
/// that differ from logical dimensions.
class DeepseekV3MultiLinear: Module {
    ...
    func callAsFunction(_ x: MLXArray, transpose: Bool = true) -> MLXArray {
        if let scales, let biases, scales.size > 1 {
            // Quantized path: derive bits and groupSize from weight shape
            let dims = inputDims
...
class DeepseekV3Attention: Module {
    ...
    @ModuleInfo(key: "embed_q") var embedQ: DeepseekV3MultiLinear
    @ModuleInfo(key: "unembed_out") var unembedOut: DeepseekV3MultiLinear
    ...
    init(config: DeepseekV3Configuration) {
        ...
        // MLA: per-head embed_q and unembed_out (replaces kv_b_proj)
        self._embedQ.wrappedValue = DeepseekV3MultiLinear(
            inputDims: qkNopeHeadDim, outputDims: kvLoraRank, numHeads: numHeads)
        self._unembedOut.wrappedValue = DeepseekV3MultiLinear(
    ...
    func callAsFunction(
        ...
        var qNope = splitQ[0]  // [B, numHeads, L, qkNopeHeadDim]
        var qPe = splitQ[1]    // [B, numHeads, L, qkRopeHeadDim]

        // Compress KV
        let compressedKvFull = self.kvAProjWithMqa(x)
        let splitCompressedKv = split(compressedKvFull, indices: [kvLoraRank], axis: -1)
        let compressedKv = splitCompressedKv[0]  // [B, L, kvLoraRank]
        var kPe = splitCompressedKv[1]           // [B, L, qkRopeHeadDim]
        kPe = kPe.reshaped(B, L, 1, self.qkRopeHeadDim).transposed(0, 2, 1, 3)

        var kvLatent = self.kvALayerNorm(compressedKv)

        // Apply RoPE
        let offset = cache?.offset ?? 0
        qPe = self.rope(qPe, offset: offset)
        kPe = self.rope(kPe, offset: offset)
        ...
        if L == 1 {
            ...
            qNope = embedQ(qNope)  // [B, H, 1, qkNopeHeadDim] → [B, H, 1, kvLoraRank]
            ...
            output = unembedOut(attnOut)  // [B, H, 1, kvLoraRank] → [B, H, 1, vHeadDim]
        } else {
            // Prefill path: project K and V from latent space
            let k = embedQ(kvLatent, transpose: false)  // [B, 1, S, kvLoraRank] → [B, H, S, qkNopeHeadDim]
            let v = unembedOut(kvLatent)                 // [B, 1, S, kvLoraRank] → [B, H, S, vHeadDim]
...
public class DeepseekV3Model: Module, LLMModel, KVCacheDimensionProvider, LoRAModel {
    ...
    init(_ args: DeepseekV3Configuration) {
        ...
        self.kvHeads = (0 ..< args.numHiddenLayers).map { _ in 1 }
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        ...
        // Handle FP8 dequantization (weight_scale_inv)
        ...
        for l in 0 ..< args.numHiddenLayers {
            ...
            // Convert kv_b_proj → embed_q + unembed_out (if kv_b_proj present)
            let attnPrefix = "\(prefix).self_attn"
            if let kvBWeight = newWeights["\(attnPrefix).kv_b_proj.weight"] {
                let isQuantized = newWeights["\(attnPrefix).kv_b_proj.scales"] != nil
                let headDim = args.qkNopeHeadDim + args.vHeadDim
                let numHeads = args.numAttentionHeads

                var v: MLXArray
                if isQuantized {
                    let kvBScales = newWeights.removeValue(
                        forKey: "\(attnPrefix).kv_b_proj.scales")!
                    let kvBBiases = newWeights.removeValue(
                        forKey: "\(attnPrefix).kv_b_proj.biases")!
                    let dims = args.kvLoraRank
                ...
                newWeights.removeValue(forKey: "\(attnPrefix).kv_b_proj.weight")

                // Reshape to [numHeads, headDim, kvLoraRank] and split
                v = v.reshaped(numHeads, headDim, -1)
                let wk = contiguous(
                    v[0..., ..<args.qkNopeHeadDim, 0...]
                        .swappedAxes(-1, -2))  // [H, kvLoraRank, qkNopeHeadDim]
                let wv = contiguous(
                    v[0..., args.qkNopeHeadDim..., 0...])  // [H, vHeadDim, kvLoraRank]
                ...
                if isQuantized {
                    ...
                    newWeights["\(attnPrefix).embed_q.weight"] = qWk
                    newWeights["\(attnPrefix).embed_q.scales"] = sWk
                    newWeights["\(attnPrefix).embed_q.biases"] = bWk
                    newWeights["\(attnPrefix).unembed_out.weight"] = qWv
                    newWeights["\(attnPrefix).unembed_out.scales"] = sWv
                    newWeights["\(attnPrefix).unembed_out.biases"] = bWv
                } else {
                    newWeights["\(attnPrefix).embed_q.weight"] = wk
                    newWeights["\(attnPrefix).unembed_out.weight"] = wv
                }
            }
        }

        // Remove weight_scale_inv keys and filter layer 61 (MTP) and rotary_emb
        return newWeights.filter { key, _ in
            !key.starts(with: "model.layers.61") && !key.contains("rotary_emb.inv_freq")
                && !key.contains("weight_scale_inv")
        }
    }
