...
public func segsum(_ x: MLXArray, mask: MLXArray? = nil) -> MLXArray {
    ...
    // Expand mask once and reuse for both the input masking and the segsum masking.
    // The original mask is [B, L]; expanding to [B, 1, L] lets it broadcast with
    // x's head dimension [B, H, L] and the segsum matrix [B, H, L, L].
    let expandedMask: MLXArray?
    if let mask = mask {
        expandedMask = MLX.expandedDimensions(mask, axis: 1)
        x = x * expandedMask!
    } else {
        expandedMask = nil
    }
