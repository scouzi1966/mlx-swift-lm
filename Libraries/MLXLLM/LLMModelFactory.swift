...
public enum LLMTypeRegistry {
    ...
    public static let shared: ModelTypeRegistry = .init(creators: [
        ...
        "glm_moe_dsa": create(GLM5MoeDsaConfiguration.self, GLM5MoeDsaModel.init),
...
public final class LLMModelFactory: ModelFactory {
    ...
    public func _load(
        ...
        let configData: Data
        do {
            let rawData = try Data(contentsOf: configurationURL)
            // Sanitize non-standard JSON values that some HuggingFace configs contain
            // (e.g. NemotronH has "time_step_limit": [0.0, Infinity])
            // Write sanitized version back to disk so the Hub library (which reads
            // config.json separately via JSONSerialization) also gets valid JSON.
            configData = Self.sanitizeJSON(rawData)
            if configData != rawData {
                try? configData.write(to: configurationURL)
            }
        } catch {
    ...
    /// Sanitize non-standard JSON values that some HuggingFace model configs contain.
    /// Python's json module handles Infinity/NaN by default, but Swift's JSONDecoder does not.
    private static func sanitizeJSON(_ data: Data) -> Data {
        guard var json = String(data: data, encoding: .utf8) else { return data }
        // Replace bare Infinity/-Infinity/NaN using regex word boundaries to handle
        // any formatting (values may be on separate lines with whitespace).
        // e.g. NemotronH has "time_step_limit": [0.0,\n        Infinity\n    ]
        json = json.replacingOccurrences(of: "\\b-Infinity\\b", with: "-1e38", options: .regularExpression)
        json = json.replacingOccurrences(of: "\\bInfinity\\b", with: "1e38", options: .regularExpression)
        json = json.replacingOccurrences(of: "\\bNaN\\b", with: "0", options: .regularExpression)
